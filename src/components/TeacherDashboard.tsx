...
export default function TeacherDashboard({
  ...
  if (selectedDetailStudentId !== null && activeStudentProfile) {
    ...
    return (
      <div id="student-detail-portal-view" className="bg-[#FAF9FE] min-h-screen pb-16 w-full overflow-x-hidden">
        ...
        <div className="max-w-md mx-auto px-5 py-5 flex flex-col gap-5">
          ...
          {/* 2. Header Siswa Card (Soft gradient matching client mockup exactly) */}
          ...
          {/* Firestore Database student details card */}
          <div className="bg-white p-5 rounded-3xl border border-stone-200/50 shadow-[0_4px_22px_rgba(0,0,0,0.015)] flex flex-col gap-3">
            <h4 className="text-xs font-extrabold text-[#1A1A1A] tracking-wider uppercase flex items-center gap-1.5 border-b border-stone-100 pb-2.5">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
              Detail Profil Siswa (Firestore)
            </h4>
            <div className="flex flex-col gap-2.5 text-xs text-stone-700">
              <div className="flex justify-between items-center py-1">
                <span className="text-stone-400 font-bold uppercase text-[9px] tracking-wider">Nama Lengkap</span>
                <span className="font-bold text-stone-950 text-right">{activeStudentProfile.name}</span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-stone-100/60">
                <span className="text-stone-400 font-bold uppercase text-[9px] tracking-wider">Kelas</span>
                <span className="font-bold text-stone-950">{activeClass?.name || activeStudentProfile.classId}</span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-stone-100/60">
                <span className="text-stone-400 font-bold uppercase text-[9px] tracking-wider">NISN</span>
                <span className="font-mono font-bold text-stone-950 bg-stone-50 px-2 py-0.5 rounded border border-stone-150">{getStudentNIS(activeStudentProfile.id)}</span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-stone-100/60">
                <span className="text-stone-400 font-bold uppercase text-[9px] tracking-wider">Program</span>
                <span className="font-bold text-stone-950 capitalize">{activeStudentProfile.program || 'REGULER'}</span>
              </div>
              <div className="flex justify-between items-center py-1 border-t border-stone-100/60">
                <span className="text-stone-400 font-bold uppercase text-[9px] tracking-wider">Wali Kelas</span>
                <span className="font-medium text-stone-950 text-right max-w-[200px] truncate" title={activeStudentProfile.wali_kelas || 'ZAMZAM IBNU SINA, M.Sc.'}>
  ...
  return (
    <div id="teacher-portal-view" className="bg-[#FAF9FE] min-h-screen pb-16 w-full overflow-x-hidden">
      ...
      <motion.div
        ...
        className="bg-white border-b border-stone-200/50 py-6 px-5 relative overflow-hidden"
      >
        {onBackToGatekeeper && (
          <button
            ...
            className="absolute left-4 top-[18px] text-stone-500 hover:text-stone-900 flex items-center gap-1.5 text-[10px] font-extrabold uppercase tracking-wider bg-stone-50 px-3 py-1.5 rounded-full border border-stone-200/50 hover:bg-stone-100 transition shadow-xs cursor-pointer z-10"
          >
            <ArrowLeft size={13} strokeWidth={2.5} /> Portal
          </button>
        )}
        ...
        <div className="max-w-md mx-auto flex flex-col items-center text-center">
          ...
          <h2 className="text-lg sm:text-xl font-black text-[#1A1A1A] tracking-tight uppercase leading-tight">
            {activeCategory} ALWILDAN 3 BSD CITY
          </h2>
        </div>
      </motion.div>
      ...
      <div className="max-w-md mx-auto px-5 py-5 flex flex-col gap-5">
        ...
        {/* Section Title "Absensi" + Informational Toggle button */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <button
            id="btn-student-info-panel-toggle"
            type="button"
            onClick={() => {
              if (selectedClassId === 'all') {
                showToastMsg('Pilih kelas terlebih dahulu untuk melihat informasi siswa.', 'info');
                return;
              }
              const classStudents = students.filter(s => s.classId === selectedClassId);
              if (classStudents.length > 0) {
                ...
                showToastMsg('Tidak ada siswa di kelas terpilih.', 'info');
              }
            }}
            className="text-[10px] font-bold tracking-wider uppercase border border-dashed border-[#FFD700] text-[#705d00] bg-[#FFD700]/10 px-3 py-1.5 rounded-full hover:bg-[#FFD700]/20 active:scale-95 transition cursor-pointer"
          >
            Informasi Siswa
          </button>
        </motion.div>
        ...
        <div className="flex flex-col gap-3">
          ...
          {/* MODE 1: DAILY TAKING MODE (Harian) */}
          {!isRangeMode ? (
                  ...
                  >
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {(['Hadir', 'Izin', 'Sakit', 'Alpa'] as AttendanceStatus[]).map((status) => {
                        ...
                        // Layout matching color chips exactly requested in theme spec:
                        // Active: Gold background with dark text
                        // Inactive: White bg with light grey/stone border
                        let styleClass = '';
                        if (isActive) {
                    ...
                    return (
                      <div id={`rekap-row-${item.student.id}`} key={item.student.id} className="pt-3.5 first:pt-0 flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                          ...
                          {/* Attendance percentage indicator widget */}
                        ...
                        {/* Breakdown status indicators badges in mini cells bar */}
